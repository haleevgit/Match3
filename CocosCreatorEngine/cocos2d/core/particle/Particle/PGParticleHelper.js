...
const ParticleHelper = cc.Class({
    name: 'pg.ParticleHelper',
    extends: cc.Component,

    editor: CC_EDITOR && {
        menu: 'Particle/ParticleHelper',
    },
    ...
    properties: {
        ...
        _isVisible: { default: true, serializable: false },
        _isDestroy: { default: true, serializable: false },

        _poolerHelper: { default: null, type: pg.PoolerHelper, serializable: false },
        _spawnPosition: {
            default() {
        ...
        _parent: { default: null, type: cc.Node, serializable: false },

        _data: { default: null, serializable: false },

        _age: { default: 0, serializable: false },

        _tempDeltaX: { default: 0, serializable: false },
        _tempDeltaY: { default: 0, serializable: false },
        //#endregion
    },

    //#region life-cycle callbacks
    onEnable() {
        this._poolerHelper = this.node.getComponent(pg.PoolerHelper);
    },
    ...
    updateColor(life) {
        if (this.data.opacity) {
            ...
            if (this.data.opacity.control) {
                this.node.opacity =
                    this.data.opacity.initial +
                    (this.data.opacity.control.value - this.data.opacity.initial) * this._getValue(this.data.opacity.control.curve, life);
            }
        }
    ...
    _getValue(control, percent) {
        ...
        const norm_percent = Math.max(percent, 0);

        let point = control[0];

        if (point.x === norm_percent) {
            return point.y;
        }
        ...
        if (last.x === norm_percent) {
            return last.y;
        }

        index = 0;

        while (point.x <= norm_percent) {
        ...
        return prev.y + ((norm_percent - prev.x) * (point.y - prev.y)) / (point.x - prev.x);
    },
    //#endregion
...
pg.ParticleHelper = module.exports = ParticleHelper;
