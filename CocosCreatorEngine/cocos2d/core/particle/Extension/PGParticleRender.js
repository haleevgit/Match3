...
const SpriteRenderHelper = cc.Class({
    ...
    properties: {
        ...
        materials: { default: [], type: [cc.Material] },
    },
    ...
    getPrefab() {
        ...
        for (const material of this.materials) {
            if (material) {
                sprite._materials.push(material);
            }
        }

        // sprite._materials = [...this.materials];

        if (this.spriteFrames.length === 1) {
            sprite.spriteFrame = this.spriteFrames[0];
        } else {
            if (this.spriteFrames.length > 1) {
                const spriteRandomHelper = node.addComponent(pg.ParticleSpriteRandomHelper);
...
const ParticleRender = cc.Class({
    ...
    properties: {
        ...
        render: {
            default() {
                return new SpriteRenderHelper();
            },
            type: RenderHelper,
            visible() {
                return this.renderType !== RenderType.None;
            },
        },
        //#endregion
        ...
        _emitter: { default: null, serializable: false }
        //#endregion
    },
    ...
    onParentChange() {
        this._emitter.updatePoolParent();
    }
    //#endregion
});
...
if (CC_EDITOR) {
    ...
    SpriteRenderHelper.template = [{ name: 'blend', type: 'number', default: 0 }];
...
ParticleRender.RenderType = RenderType;
ParticleRender.BlendType = BlendType;
ParticleRender.RenderHelper = RenderHelper;
ParticleRender.PrefabRenderHelper = PrefabRenderHelper;
ParticleRender.SpriteRenderHelper = SpriteRenderHelper;

pg.ParticleRender = module.exports = ParticleRender;
